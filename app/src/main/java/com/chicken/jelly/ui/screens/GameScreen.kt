...
import com.chicken.jelly.ui.components.HowToPlayOverlay
...
fun GameScreen(viewModel: GameViewModel, onExit: () -> Unit, soundManager: SoundManager) {
    ...
    LaunchedEffect(Unit) { soundManager.playGameMusic() }
        ...
        Column(modifier = Modifier.fillMaxSize()) {
            ...
            Box(modifier = Modifier.weight(1f)) {
                ...
                if (state.showTutorial && !state.isPaused) {
                    HowToPlayOverlay(
                        fontFamily = font,
                        onStart = { viewModel.startRun(soundManager) }
                    )
                }
            }
            Row(
